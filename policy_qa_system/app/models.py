...
# 用户反馈相关函数
def add_user_feedback(user_id, feedback_type, content):
    """添加用户反馈"""
    conn = sqlite3.connect(DATABASE_PATH)
    cursor = conn.cursor()
    try:
        cursor.execute('''
            INSERT INTO user_feedback (user_id, type, content)
            VALUES (?, ?, ?)
...
def get_all_feedback():
    ...
    cursor = conn.cursor()
    cursor.execute('''
        SELECT uf.id, uf.user_id, uf.type, uf.content, uf.status, uf.created_at, u.nickname
    ...
    conn.close()
    return [{
        'id': f[0], 'user_id': f[1], 'type': f[2], 'content': f[3],
        'status': f[4], 'created_at': f[5], 'nickname': f[6]
    } for f in feedbacks]
...
def init_default_roles_and_permissions():
    ...
    try:
        ...
        permissions = [
            ...
            ('forum_manage', '管理讨论'),
            ('expert_manage', '管理专家'),
            ('interpretation_manage', '管理解读'),
            ('public_voice_manage', '管理民声'),
            ('feedback_manage', '管理反馈'),
        ...
        # 为角色分配权限
        role_permissions = {
            'user': ['policy_view', 'forum_view', 'forum_post'],
            'admin': ['policy_view', 'policy_add', 'policy_edit', 'policy_delete',
                      'user_view', 'user_edit', 'user_ban', 'forum_view', 'forum_post',
                      'forum_manage', 'expert_manage', 'interpretation_manage',
                      'public_voice_manage', 'feedback_manage', 'statistics_view', 'rag_manage'],
            'expert': ['policy_view', 'forum_view', 'forum_post', 'interpretation_manage']
        }
...
def check_user_permission(user_id, permission_name):
    ...
    try:
        ...
        if not user_role:
            # 如果用户没有关联角色，使用旧的role字段
            cursor.execute('SELECT role FROM users WHERE id = ?', (user_id,))
            old_role = cursor.fetchone()
            if old_role:
                user_role_name = old_role[0]
            else:
                return False
        else:
        ...
        ''', (user_role_name, permission_name))
        count = cursor.fetchone()[0]
        return count > 0
    except Exception as e:
